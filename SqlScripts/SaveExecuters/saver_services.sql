create table murr_downloader.saver_tasks
(
	task_id bigint NOT NULL DEFAULT nextval('murr_downloader.murr_sequence'::regclass),
	task_type_id bigint,
	saver_template_id bigint,
	task_status_id bigint,
	constraint pk_saver_tasks_task_id primary key(task_id),
	CONSTRAINT fk_saver_tasks_task_statuses_tasks_status_id FOREIGN KEY (task_status_id)
        REFERENCES murr_downloader.task_statuses (task_status_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	CONSTRAINT fk_saver_tasks_saver_templates_saver_template_id FOREIGN KEY (saver_template_id)
        REFERENCES murr_downloader.saver_templates (saver_template_id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE,
	constraint fk_saver_tasks_task_types_task_type_id FOREIGN KEY (task_type_id)
...
COMMENT ON TABLE murr_downloader.tasks
    IS 'Задачи для загрузки в БД';


CREATE OR REPLACE FUNCTION murr_downloader.tasks_copy() RETURNS TRIGGER AS
$BODY$
declare 
	d_task_type_id bigint;
BEGIN 
	--BEGIN
	if(new.task_status_id = 4) then 
	
		select task_type_id into d_task_type_id
		from murr_downloader.task_templates
		where new.task_template_id = task_template_id;

		INSERT INTO
			murr_downloader.saver_tasks(task_type_id, saver_template_id, task_status_id)
			VALUES(d_task_type_id, new.saver_template_id, 1);
	end if;

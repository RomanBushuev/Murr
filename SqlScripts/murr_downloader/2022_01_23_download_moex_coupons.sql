...
values(9, 'DOWNLOAD MOEX COUPONS', 'Загрузка купонов из Moex');

CREATE OR REPLACE FUNCTION murr_downloader.add_moex_coupons()
    RETURNS bigint
    LANGUAGE 'plpgsql'
    COST 100
...
declare
	...
	d_folder_sub_root character varying(255) = 'MOEX_COUPONS';
...
begin
	...
	insert into murr_downloader.task_templates(task_template_title, 
		...
		task_parameters, 
		task_type_id) values(d_title, now()::timestamp without time zone, d_folder_id, d_task_parameters, 9)
...
ALTER FUNCTION murr_downloader.add_moex_coupons()
    OWNER TO karma_admin;

GRANT EXECUTE ON FUNCTION murr_downloader.add_moex_coupons() TO karma_admin;
GRANT EXECUTE ON FUNCTION murr_downloader.add_moex_coupons() TO karma_downloader;
REVOKE ALL ON FUNCTION murr_downloader.add_moex_coupons() FROM PUBLIC;
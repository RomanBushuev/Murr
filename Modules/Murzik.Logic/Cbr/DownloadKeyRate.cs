...
{
    public class DownloadKeyRate : Algorithm
    {
        ...
        public DownloadKeyRate(ILogger logger, 
        ...
        public override async Task Run()
        {
            ...
            try
            {
                Log.Info($"Задача {TaskId} : Задача загрузки ключевой ставки из ЦБ начата");
                IsContinue();
                DateTime dateTime = _paramDescriptors.ConvertDate(RunDateTime);
                Log.Info($"Задача {TaskId} : загрузка ключевой ставки за дату {dateTime.ToShortDateString()}");
                var xmlDocument = await _cbrDownloader.DownloadKeyRate(dateTime);
                Log.Info($"Задача {TaskId} : загрузка ключевой ставки завершена");
                ...
                Log.Info($"Задача {TaskId} : Задача загрузки ключевой ставки из ЦБ закончена");
            }
